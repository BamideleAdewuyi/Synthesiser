...
#endif
{
    // When a note is pressed, we want garbage/samples from previous key press to be cleared
    mySynth.clearVoices();
    
    // This is for a synth with 5 voices
    for (int i = 0; i < 5; i++)
    {
...
{
    // ignoreUnused clears samples from last key press
    juce::ignoreUnused(samplesPerBlock);
    // Initially, this is the sample rate
...
void SynthFrameworkAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    // mySynth.renderNextBlock(AudioBuffer<float> &outputAudio, const MidiBuffer &inputMidi, int startSample, int numSamples)
